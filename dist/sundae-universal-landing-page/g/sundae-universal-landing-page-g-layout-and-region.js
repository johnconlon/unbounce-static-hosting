(function() {
  ...
  function bindFormRedirectOnSubmit(args) {
    ...
    args.formElement.querySelector(
      "input[name='in_region']"
    ).value = isInRegion;
  }
  ...
  function setFlexibleFormFieldSize() {
    if ($(window).width() > 600) {
      ...
      $(".lp-pom-form .lp-pom-form-field").each(function(i, field) {
        ...
        $(this)
          .children()
          .outerWidth($(boxes[i]).outerWidth());
        $(this)
          .children()
          .outerHeight($(boxes[i]).outerHeight());
        // Hide box
  ...
  lp.jQuery(document).ready(function($) {
    ...
    window.ub.hooks.beforeFormSubmit.push(bindFormRedirectOnSubmit);
  });
